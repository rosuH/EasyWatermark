...
2. **Compose Multiplatform / KMP migration** (phases C1–C6): `docs/superpowers/plans/2026-06-12-cmp-migration-plan.md`. Decisions in `docs/adr/`; domain vocabulary in `docs/CONTEXT.md`; dependency-level KMP classifications in `docs/superpowers/research/2026-06-12-cmp-readiness-audit.json`. **Foundations now landed (2026-06-13), not just planned:**
   - **`:shared` KMP module EXISTS** — `kotlin-multiplatform` with `androidTarget()` + `jvm("desktop")`; pure-Kotlin only (no Compose/Android resources yet → sidesteps CMP-9547). `commonMain` holds platform-neutral domain types `ImageFormat`, `Result`, `JobState`, compiling for **Android + JVM/desktop** and consumed by `:app`. Add new platform-neutral domain types here; iOS targets + the commonMain renderer come in C5/C2.
