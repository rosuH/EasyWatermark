...
2. **Compose Multiplatform / KMP migration** (phases C1–C6): `docs/superpowers/plans/2026-06-12-cmp-migration-plan.md`. Decisions in `docs/adr/`; domain vocabulary in `docs/CONTEXT.md`; dependency-level KMP classifications in `docs/superpowers/research/2026-06-12-cmp-readiness-audit.json`. **Foundations now landed (2026-06-13), not just planned:**
   ...
   - **C2a STARTED (text-cell sizing wired, verified):** `WaterMarkImageView.buildTextBitmapShader` now delegates cell sizing (rotated-AABB + gap) to commonMain `WatermarkGeometry` — verified on-device as render-identical (goldens + S22+ preview parity). The live Android renderer and the JVM/iOS targets now share that sizing core. (`calculateMaxSize`/icon path + the full Canvas composition are NOT yet shared — still per-platform.)
   - **Still ahead:** wire the remaining cell/composition paths (icon, the offscreen compose/export) into commonMain; the Compose-`Canvas` preview swap that deletes `ViewInfo`/`WaterMarkImageView` (C2b); `:desktopApp` is a runnable scaffold, not the Compose Desktop editor (C4); iosApp bring-up (PHPicker, photo store, memory, Compose UI) is C5.
