...
- Next S4d work should establish Android-vs-commonMain parity evidence before any draw-swap. The likely gate is a test-only Android adapter/pixel comparison for text and icon cells, plus an explicit bundled-font decision for CJK/emoji and a separate decision on when to start REPEAT/CLAMP tiling in commonMain.
