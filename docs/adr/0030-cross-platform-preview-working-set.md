...
- Android focus Source is a strong repository entry so `TRIM_MEMORY_UI_HIDDEN` can drop Watermarked / neighbors without forcing three `openInputStream`s on a brief app-switch. `TRIM_MEMORY_BACKGROUND` (process is a kill candidate) evicts the whole working set — every frame is reconstructable. Both go through `AndroidMemoryPressure`.
