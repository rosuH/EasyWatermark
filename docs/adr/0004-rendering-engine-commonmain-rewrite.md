...
- **CJK measurement (candidate, pending C2b sign-off):** the commonMain `TextMeasurer` seam is byte-exact vs legacy `StaticLayout` for the covered Latin/emoji/bold/italic rows, but **CJK cell height grows** (width stays exact) — a Compose line-height difference that scales with text size and line count (device SM-S906E/API 36: single-line +1/+2/+5px at 12/24/48f; two-line +4/+9/+18px). The candidate direction is to **accept the Compose CJK line-height as the C2b future-renderer baseline** under this ADR's `textSize` re-spec + golden re-baseline (Risk R4), rather than fragile per-script compat tuning — gated by an instrumented signed-baseline test (`WatermarkCellParityGateTest`, ADR-0010/0014). Not yet wired into the product renderer/export; final acceptance is the C2b sign-off.
