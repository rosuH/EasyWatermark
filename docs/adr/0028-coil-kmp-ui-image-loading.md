...
- **iOS/Desktop:** Fetcher/decode from path/`MediaRef` with **maxEdge** sampling, never full-res for UI thumbs.  
  ...
  **iOS amendment (2026-08-14):** the ImageIO thumbnail path adds `kCGImageSourceSubsampleFactor`, gated per policy (`ProductUi.allowSubsample = true`, `Preview` unchanged). `kCGImageSourceCreateThumbnailFromImageAlways` decodes the **full** image before scaling, which is why a 128px album-HEIC thumbnail measured *more* expensive than a 1920px preview (device: `io128_med` 202 ms vs `io1920_med` 138 ms). The subsample factor is the largest of 8 / 4 / 2 that keeps the subsampled long edge at or above the requested edge, so output dimensions are unchanged — verified by `IosImageIOSubsampleTest`. `kCGImageSourceCreateThumbnailFromImageIfAbsent` was **rejected**, not merely unused: it returns the embedded thumbnail at an unpredictable and possibly tiny size, so it is not a safe drop-in.  
