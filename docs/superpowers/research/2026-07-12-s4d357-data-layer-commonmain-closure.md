...
| `MemorySettingRepo` | `app/src/main/java/me/rosuh/easywatermark/data/repo/MemorySettingRepo.kt`; DI `app/src/main/java/me/rosuh/easywatermark/di/RepositoryModule.kt`; constructor-injected into `app/src/main/java/me/rosuh/easywatermark/ui/MainViewModel.kt` and `app/src/main/java/me/rosuh/easywatermark/ui/about/AboutViewModel.kt` | **Empty class** (no members, no behavior). DI-injected by `MainViewModel` and `AboutViewModel` but unused for state. **Not** a dual-platform pure extraction candidate. |
