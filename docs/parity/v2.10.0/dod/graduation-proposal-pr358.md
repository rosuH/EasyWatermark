...
- Shared multi-target compile; desktopTest **132/0**.  
...
|----------|-------------------|
| Strict FNV goldens not re-run on pinned host | Medium — local gate only |
| Full `iosSimulatorArm64Test` RUN not in final audit | Medium — re-run before ship if desired |
| Parity archive not exhaustive (locale/theme/recording matrix) | Medium — core screens signed |
| `codex-goal-v2.md` §11 stale vs ticket completion | Low — process docs refresh |
...
### Option A — **Keep Draft; continue polish** (Recommended if residual medium items matter)

1. Optional: re-run strict goldens + `iosSimulatorArm64Test` on lab host.  
2. Optional: expand parity archive (zh / theme / recordings) for residual inventory rows.  
3. Refresh `codex-goal-v2.md` §11 “current truth” after accept.  
4. Push branch when owner authorizes; keep #358 Draft until residuals closed.  
...
1. Owner accepts **PARTIAL** §9 with residual list as known debt.  
...
5. Track residuals as post-merge tickets (strict goldens, archive expansion, §11 docs).

### Option C — **Not ready; no graduation**

1. Leave Draft.  
2. Open only residual work (e.g. strict goldens, full locale parity).  
...
**Option A** (or **B** only if owner accepts residual debt in writing).  
**§9 NOT MET** → **do not merge** on the strength of this audit alone.  
Present this proposal as the ticket **10** deliverable for owner decision.
