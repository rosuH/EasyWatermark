...
- **CommonMain raster** (`CommonWatermarkPipeline` + `WatermarkCellComposer` primitives): Android production via `AndroidCommonRaster` (ADR-0018). Desktop production via `DesktopWatermarkComposer.composeRealImage` + `DesktopRenderRequest` (bundled Latin+CJK family + frozen per-item offset; Stage C2 / issue 21). Desktop still owns decode/encode/destination/direct write. iOS still orchestrates primitives directly until Stage C3. Never claim byte-parity with legacy native goldens; rebaseline per `docs/adr/0010-c2-golden-policy-delta.md`.
...
- Desktop: `./gradlew :desktopApp:run` (window), `:desktopApp:run --args='--headless'` (automation), `:desktopApp:createDistributable` (unsigned `.app`; needs a supported JDK — Corretto/Zulu, NOT Homebrew OpenJDK, CMP#3107).
- No linter is wired up (spotless/ktlint blocks in root `build.gradle.kts` are commented out). Match existing style by hand.
- Release builds are minified (R8, `proguard-rules.pro` + `coroutines.pro`); CI (`.github/workflows/`) runs `:app:assembleDebug` + `:shared:desktopTest` + non-strict `:app:testDebugUnitTest` on PRs, signing + Play upload on release.
