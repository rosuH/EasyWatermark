...
- Desktop: `./gradlew :desktopApp:run` (window), `:desktopApp:run --args='--headless'` (automation), `:desktopApp:createDistributable` (unsigned app image; needs Corretto/Zulu, NOT Homebrew OpenJDK, CMP#3107). Packaged Dock/installer icon is `nativeDistributions.iconFile` (`desktopApp/icons/` icns/ico/png); `Window(icon)` is the runtime window icon — `gradle run` Dock may still be the Java process. **J3:** app-data is OS-native (`DesktopAppPaths`) with legacy `~/.easywatermark` copy-forward; chooser formats via `DesktopImageFormats` (WebP only if ImageIO can decode). Unsigned `createDistributable` is **not** a signed three-OS release (DMG/MSI/DEB + signing residual).
