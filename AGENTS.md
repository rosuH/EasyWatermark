...
4. **Notify the right Claude Code worker.** Never rely on the focused terminal. Enumerate terminal sessions/processes and match by `claude` process, cwd, TTY, and EasyWatermark/session screen content. iTerm2 cannot be targeted reliably through Computer Use; use the AppleScript fallback after matching the correct tab. Codex side terminals can be checked through the terminal buffer plus process/TTY. After sending, verify the session moves from `inbox/` to `active/` or that the worker visibly starts reading it.
5. **Poll by filesystem state and terminal evidence.** For ACSP sessions, inspect `inbox/`, `active/`, `review/`, `blocked/`, `failed/`, and `done/`; read `events.jsonl`, `messages.jsonl`, heartbeat/status files, and the visible worker terminal. Do not send extra instructions unless the current thread asks for them, the session is clearly stuck, or a review decision requires a requeue.
