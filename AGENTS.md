...
- Release builds are minified (R8, `proguard-rules.pro` + `coroutines.pro`); CI (`.github/workflows/pr_pre_check.yml`) on PRs: Ubuntu `build` (`:app:assembleDebug` + `:shared:desktopTest` + non-strict `:app:testDebugUnitTest`) and permanent macOS `ios` job (J1: `:shared:iosSimulatorArm64Test` + `iosApp` generic iOS Simulator `xcodebuild`, `CODE_SIGNING_ALLOWED=NO`). Signing + Play upload on release.
