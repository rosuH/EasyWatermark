 ...
 * The Android edge, exactly as the legacy `Int?.toTileMode()` gated it (DECAL only on API >= S, else * REPEAT).
 *
 ...
 */
/**
 * Android render/edge bridge for [WaterMark]. The neutral [WaterMark] model (commonMain) holds
 * [WatermarkTileMode]; render/export code still asks for an `android.graphics.Shader.TileMode`. This was
 * The former `WaterMark.obtainTileMode()` member — kept as an extension so all call sites * (`WatermarkRenderer`, `MainViewModel.generateImage`, `EditorScreen`) are unchanged aside from the import.
 */
 ...
 * The historical `Int?.toTileMode()`, so the **neutral model value the UI observes** matches the old * behavior — not just the rendered `Shader.TileMode`:
 *
