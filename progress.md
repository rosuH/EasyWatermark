...
- **C2a engine-wiring attempt → REVERTED (important learning).** Tried delegating `buildTextBitmapShader`'s cell sizing (gap helpers + rotated-AABB) to commonMain `WatermarkGeometry` — formulas character-identical, the dimension golden stayed 93×33, and there was NO logcat crash. **But the live editor preview went blank on the real device.** Confirmed causal: stashing the wiring restored the image (pre-wiring screencap 2.3MB vs wired 53KB blank). Reverted (discarded) to protect the product core; committed state `e09ff00` renders correctly.
...
- **C2a status (unchanged):** the live-preview blank from the reverted C2a wiring remains undiagnosed — but it is NOT a cell-output blank (the cell renders fine in both goldens incl. emoji/315). The cause is in the View-integration path; a focused session should re-introduce the wiring + bisect with these goldens + on-device preview screencaps.
