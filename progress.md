...
- **Pixel golden + a CRITICAL C1.7 finding about Robolectric (`efa722b`/`133da5f`).** Upgraded the golden to pixel-level: composites the tiled cell and pins rendered output (ASCII "GOLDEN"@0°: 1086 visible px + signature) — catches blank-render regressions the dimension golden missed. **But:** the REAL default config (emoji "👋 DO NOT REDISTRIBUTE" @315°) renders **BLANK (0 px) under Robolectric NATIVE**, though it renders fine on-device. ⇒ **Robolectric NATIVE is NOT a faithful oracle for this app's actual watermark (emoji + rotation).** The device-free JVM golden covers simple ASCII/0° cells only; a faithful golden of the real configs — and trustworthy C2a-swap verification — **requires INSTRUMENTED (on-device) tests** (pinned as a passing "known gap" test). This refines the plan's C1.7 (which leaned toward a JVM/Robolectric golden) and likely explains the C2a blank-preview: my ASCII/0° golden never exercised emoji/315°, so it couldn't catch a degree-315/emoji-specific break. **Next C2a session: build the golden on an instrumented (device) harness for the real configs, then bisect the wiring.**
